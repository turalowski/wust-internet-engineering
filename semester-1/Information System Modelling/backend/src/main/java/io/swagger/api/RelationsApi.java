...
public interface RelationsApi {
    ...
    @ApiResponses(value = { 
        @ApiResponse(responseCode = "200", description = "relation response", content = @Content(mediaType = "application/json", schema = @Schema(implementation = NewItem.class))),
    ...
    ResponseEntity<NewItem> addRelation(@Parameter(in = ParameterIn.DEFAULT, description = "Add relation", required=true, schema=@Schema()) @Valid @RequestBody Relation body);
    ...
    @ApiResponses(value = { 
        @ApiResponse(responseCode = "200", description = "relation response", content = @Content(mediaType = "application/json", schema = @Schema(implementation = GetRelation.class))),
    ...
    ResponseEntity<GetRelation> getRelation(@Parameter(in = ParameterIn.PATH, description = "ID of relation to fetch", required=true, schema=@Schema()) @PathVariable("id") Long id);


    @Operation(summary = "", description = "This endpoint returns all relations from the database.", tags={ "relations" })
    @ApiResponses(value = { 
        @ApiResponse(responseCode = "200", description = "Returns list of relations.", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = GetRelation.class)))),
        
        @ApiResponse(responseCode = "200", description = "This is default error response for the endpoint. For different error codes, different error messages will be generated.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorModel.class))) })
    @RequestMapping(value = "/relations",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<List<GetRelation>> getRelations(@Parameter(in = ParameterIn.QUERY, description = "" ,schema=@Schema()) @Valid @RequestParam(value = "type", required = false) BigDecimal type, @Parameter(in = ParameterIn.QUERY, description = "" ,schema=@Schema()) @Valid @RequestParam(value = "name", required = false) String name);
